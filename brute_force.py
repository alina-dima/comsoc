...
def manipulate(orders, alternatives, winner, not_winner_idxs):
    ...
    for v in tqdm(range(1, len(not_winner_idxs))):
        for voters in tqdm(itertools.combinations(not_winner_idxs, v)):
            # print(voters)
            
            # Generate all possible orders with any length
            for r in range(1, 12):
                for permutation in itertools.permutations(range(1, 12), r):
                    # print(permutation)
